 ...
 * into native Unicode Bold & Italic characters for LinkedIn, X, and Instagram previews.
 */
...
export function convertMarkdownToUnicode(text: string): string {
  ...
  let converted = text.replace(/\*\*(.*?)\*\*|__(.*?)__/g, (_match, p1, p2) => {
    const inner = p1 || p2;
    return toUnicodeBold(inner);
  });
  
  // Convert *italic* or _italic_ to Unicode Italic
  converted = converted.replace(/\*(.*?)\*|_(.*?)_/g, (_match, p1, p2) => {
    const inner = p1 || p2;
